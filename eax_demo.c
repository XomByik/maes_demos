...
bool parse_next_test_case(FILE *fp, TestCaseData *data) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (!trimmed || strlen(trimmed) == 0 || trimmed[0] == '#') {
            if (in_test_case && data->key_hex && data->nonce_hex && data->pt_hex && data->ct_hex) {
        ...
        switch(type) {
            ...
            case CIPHER:
                ...
                if (value && strlen(value) >= 32) { // Predpokladáme 16B tag (32 znakov hex)
                    ...
                    // Ak CIPHER nie je dostatočne dlhý na tag, predpokladajme že celé je tag
                    data->tag_hex = value;
                    data->ct_hex = my_strdup("");
                    value = NULL;
                }
                free(value);

                // Po spracovaní CIPHER skontrolujeme, či máme kompletný testovací prípad
                if (data->key_hex && data->nonce_hex && data->tag_hex) {
                    // Ak nemáme plaintext, nastavíme prázdny
                    if (!data->pt_hex) {
                        data->pt_hex = my_strdup("");
                    }
                    data->should_fail = fail_tag_seen;
                    if (data->count == 0) {
                        data->count = ++current_count;
...
bool process_test_case(const TestCaseData *data, int *passed_encrypt, int *passed_decrypt) {
    ...
    if (data->pt_hex) {
        ...
        if (result_combined_hex) {
            ...
            for (size_t i = 0; i < lens[5]; i++) {
                sprintf(result_combined_hex + (lens[3] * 2) + (i * 2), "%02x", result_tag[i]);
            }
        ...
        bool ct_match = (!data->ct_hex || lens[4] == 0 || memcmp(result_ct, bufs[4], lens[4]) == 0);
    ...
    if (data->ct_hex) {
        ...
        int decrypt_stav = AES_EAX_decrypt(
            bufs[0], bufs[1], combined_ct_tag, 
            lens[4], bufs[2], lens[2], lens[5], decrypted
        );
        printf("  Ocakavany stav desifrovania: %s\n", 
            data->should_fail ? "ZLYHANIE (Tag neplatny)" : "USPESNE (Tag platny)");
        
        printf("  Skutocny stav desifrovania: %s\n", 
               decrypt_stav == 0 ? "USPSNE (Tag platny)" : "ZLYHANIE (Tag neplatny)");
