#include "ofb_config.h"

typedef enum {
    KEY, IV, BLOCK, INPUT_BLOCK, OUTPUT_BLOCK, PLAINTEXT, CIPHERTEXT, MODE_CHANGE, UNKNOWN
} LineType;
...
static LineType get_line_type(const char* line) {
    ...
    if (strstr(line, "OFB") != NULL) return MODE_CHANGE;
    return UNKNOWN;
}
        ...
        switch(type) {
            case MODE_CHANGE:
                current_mode = (strstr(trimmed, "Encrypt") != NULL) ? 1 : 
                               (strstr(trimmed, "Decrypt") != NULL) ? 2 : 0;
                
                printf("\n--- Testovanie %s ---\n", 
                      (current_mode == 1) ? "sifrovania (Encrypt)" : 
                      (current_mode == 2) ? "desifrovania (Decrypt)" : "neznameho rezimu");
                break;
                
            case KEY:
                free(hex_key);
                hex_key = my_strdup(trim(trimmed + 4));
                if (hex_to_bin(hex_key, key, AES_KEY_SIZE) != 0) {
                    fprintf(stderr, "Error parsing Key hex.\n");
                    free(hex_key); hex_key = NULL;
                } else {
                    printf("\nKluc: %s\n", hex_key);
                }
                break;
            ...
            case IV:
                free(hex_iv);
                hex_iv = my_strdup(trim(trimmed + 3));
                break;
            ...
            case INPUT_BLOCK:
                ...
                hex_input_block = my_strdup(trim(trimmed + 12));
                
                if (block_number > 0 && block_number <= OFB_MAX_BLOCKS) {
                    TestVector *target = (current_mode == 1) ? &encrypt_tests[block_number-1] : 
                                       (current_mode == 2) ? &decrypt_tests[block_number-1] : NULL;
                    
                    if (target) {
                        strncpy(target->hex_input_block, hex_input_block, OFB_HEX_BUFFER_SIZE-1);
            ...
            case OUTPUT_BLOCK:
                ...
                hex_output_block = my_strdup(trim(trimmed + 13));
                ...
                if (block_number > 0 && block_number <= OFB_MAX_BLOCKS) {
                    ...
                    if (target) {
                        strncpy(target->hex_output_block, hex_output_block, OFB_HEX_BUFFER_SIZE-1);
                    }
                }
                break;
            ...
            case PLAINTEXT:
                free(hex_plaintext);
                hex_plaintext = my_strdup(trim(trimmed + 10));
                ...
                if (block_number > 0 && block_number <= OFB_MAX_BLOCKS) {
                    ...
                    if (target) {
                        strncpy(target->hex_plaintext, hex_plaintext, OFB_HEX_BUFFER_SIZE-1);
                    }
                }
                break;
            ...
            case CIPHERTEXT:
                free(hex_ciphertext);
                hex_ciphertext = my_strdup(trim(trimmed + 11));
                ...
                if (block_number > 0 && block_number <= OFB_MAX_BLOCKS) {
                    ...
                    if (target) {
                        strncpy(target->hex_ciphertext, hex_ciphertext, OFB_HEX_BUFFER_SIZE-1);
                    }
                }
                break;
...
bool process_ofb_test_case(uint8_t *key, TestVector *test, int *passed_count, int test_count, bool is_encrypt) {
    ...
    uint8_t input[OFB_MAX_DATA_SIZE], expected[OFB_MAX_DATA_SIZE], result[OFB_MAX_DATA_SIZE];
...
int main() {
    ...
    if (!parse_test_vectors(fp, encrypt_tests, decrypt_tests, &encrypt_test_count, 
                       &decrypt_test_count, key)) {
    ...
    if (encrypt_test_count > 0) {
        ...
        for (int i = 0; i < encrypt_test_count; i++) {
            test_count++;
            process_ofb_test_case(key, &encrypt_tests[i], &passed_count, test_count, true);
        }
    }
    ...
    if (decrypt_test_count > 0) {
        ...
        for (int i = 0; i < decrypt_test_count; i++) {
            test_count++;
            process_ofb_test_case(key, &decrypt_tests[i], &passed_count, test_count, false);
        }
    }
