 ...
 * on dynamic computation, refer to https://en.wikipedia.org/wiki/Rijndael_S-box
 */
...
{
    const uint8_t s = dataSize ? (dataSize - 1) % BLOCKSIZE + 1 : 0, *k = K1;
                      ...
                      const void* pntxt, const size_t ptextLen, void* crtxt )
{
    ...
    if (n == 0)  return ENCRYPTION_FAILURE;      /*  data size >= BLOCKSIZE   */
#endif
...
#else
    if (r != 0)  return DECRYPTION_FAILURE;
#endif
...
#else
    block_t BCOUNTER = { 0 };
    memcpy( BCOUNTER, iv, CTR_IV_LENGTH );
    xorBEint( BCOUNTER, CTR_STARTVALUE, LAST );  /*  initialize the counter   */
#endif

    AES_SetKey( key );
    CTR_Cipher( BCOUNTER, 1, pntxt, ptextLen, crtxt );
    BURN( RoundKey );
}
 ...
 * @param   sectid    sector id: use this instead of tweak if it was null
 * @param   data      the input plain/cipher-text
...
static void XTS_Cipher( const uint8_t* keypair, const char mode,
                        ...
                        const void* data, const size_t dataSize, void* storage )
{
    ...
    memcpy( storage, data, dataSize );           /* do in-place en/decryption */

    if (tweak == NULL)
    {                                            /* the `i` block is either   */
        memset( T, 0, BLOCKSIZE );               /* ..a little-endian number  */
        copyLint( T, sectid, 0 );                /* ..or a byte array (tweak) */
    }
    else
    {
        memcpy( T, tweak, BLOCKSIZE );
    }
...
char AES_XTS_encrypt( const uint8_t* keys, const uint8_t* tweak,
                      const void* pntxt, const size_t ptextLen, void* crtxt )
{
    if (ptextLen < BLOCKSIZE)  return ENCRYPTION_FAILURE;

    XTS_Cipher( keys, 1, tweak, 0, pntxt, ptextLen, crtxt );
    return NO_ERROR_RETURNED;
}
...
char AES_XTS_decrypt( const uint8_t* keys, const uint8_t* tweak,
                      const void* crtxt, const size_t crtxtLen, void* pntxt )
{
    if (crtxtLen < BLOCKSIZE)  return DECRYPTION_FAILURE;

    XTS_Cipher( keys, 0, tweak, 0, crtxt, crtxtLen, pntxt );
    return NO_ERROR_RETURNED;
}
#endif /* XTS */
                 ...
                 const size_t aDataLen, const size_t ptextLen, block_t IV )
{
     ...
     * e.g. let aData be a 2D array and aDataLen an array with size NA that none
     * of its elements are zero. then we could write something like this instead
     * for (i = 0; i < NA; ++i) { cMac( *K, Q, aData[i], aDataLen[i], IV )... */
    if (aDataLen)
    {
                      ...
                      const block_t Ld, const block_t delta0, block_t delta )
{
    ...
    while ((m = index - b) < index)
    {
        doubleBblock( L );                       /*  L_0 = double( L_$ )      */
        b *= 2;                                  /*  L_i = double( L_{i-1} )  */
        if ((m & (b * 2 - 1)) < b)
        {
 ...
 * @param   pSize     size of plaintext. psize = dataSize only in encryption
 * @param   aDataLen  size of additional authentication data
 * @param   aData     additional authentication data
 * @param   dataSize  size of input data, to be encrypted/decrypted
                        ...
                        const size_t dataSize, void* data, block_t tag )
{
    ...
    if ((n = dataSize % BLOCKSIZE) != 0)
    {                                            /*  Y_* = Enc(L_* ^ Δ_N) ^ X */
        tag[n] ^= 0x80;                          /*  and pad X_* or checksum  */
        xorBlock( Ls, del );
...
static void addRdx( const rbase_t* q, const size_t len, rbase_t* p )
{
    size_t i, a = 0;
...
static void subRdx( const rbase_t* q, const size_t len, rbase_t* p )
{
    size_t i, s = 1;
                        ...
                        rbase_t* X )
{
    ...
    {                                            /*  encryption rounds        */
        FF1round( i++, P, u, len, Xc );          /*  A := Xc-len;  B := Xc-u  */
        addRdx( Xc, u, Xc - (i & 1 ? len : u) );
    }
    for (i ^= 10; i > (0); u = len - u)          /*  decryption rounds        */
    {
        FF1round( --i, P, u, len, Xc );
        subRdx( Xc, u, Xc - (i & 1 ? u : len) );
    }
}
#else                                            /*         FF3/FF3-1         */
...
/** add two numbers in base-RADIX represented by q and p, so that: p = p + q; */
static void addRdx( const rbase_t* q, const uint8_t len, rbase_t* p )
{
    size_t i, a = 0;
...
/** subtract two numbers in base-RADIX represented by q and p, so that p -= q */
static void subRdx( const rbase_t* q, const uint8_t len, rbase_t* p )
{
    size_t i, s = 1;
                        ...
                        const uint8_t len, const uint8_t* tweak, rbase_t* X )
{
    ...
    {                                            /*  encryption rounds        */
        FF3round( i++, T, u, len, Xc );          /*  A := Xc-len;  B := Xc-u  */
        addRdx( Xc, u, Xc - (i & 1 ? len : u) );
    }
    for (i ^= 8; i > (0); u = len - u)           /*  decryption rounds        */
    {
        FF3round( --i, T, u, len, Xc );
        subRdx( Xc, u, Xc - (i & 1 ? u : len) );
    }
}
#endif /* FF_X */
