...
typedef enum {
    KEY_T, AD_T, PT_T, CT_T, CMAC_T, IV_C_T, COUNT_T, FAIL_T
} LineType;

static LineType get_line_type(const char* line) {
    if (strstr(line, "Key:") || strstr(line, "Key = ")) return KEY_T;
    if (strstr(line, "AD:") || strstr(line, "AD = ")) return AD_T;
    if (strstr(line, "Plaintext:") || strstr(line, "Plaintext = ")) return PT_T;
    if (strstr(line, "Ciphertext:") || strstr(line, "Ciphertext = ")) return CT_T;
    if (strstr(line, "CMAC(final):")) return CMAC_T;
    if (strstr(line, "IV || C:")) return IV_C_T;
    if (strstr(line, "Count = ")) return COUNT_T;
    if (strstr(line, "FAIL")) return FAIL_T;
    return -1;
}
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        switch(type) {
            ...
            case KEY_T:
                ...
                if (!value) value = get_line_value(trimmed, "Key = ");
                if (!data->hex_key) data->hex_key = value; else free(value);
                break;

            case AD_T:
                value = get_line_value(trimmed, "AD:");
                if (!value) value = get_line_value(trimmed, "AD = ");
                if (!data->hex_ad) data->hex_ad = value; else free(value);
                break;

            case PT_T:
                value = get_line_value(trimmed, "Plaintext:");
                if (!value) value = get_line_value(trimmed, "Plaintext = ");
                if (!data->hex_plaintext) data->hex_plaintext = value; else free(value);
                break;

            case CT_T:
                value = get_line_value(trimmed, "Ciphertext:");
                if (!value) value = get_line_value(trimmed, "Ciphertext = ");
                if (!data->hex_expected_ct) data->hex_expected_ct = value; else free(value);
                break;

            case CMAC_T:
                value = get_line_value(trimmed, "CMAC(final):");
                if (!data->hex_expected_iv) data->hex_expected_iv = value; else free(value);
                break;
...
bool process_test_case(const TestCaseData *data, int *passed_encrypt, int *passed_decrypt) {
    if (!data->hex_key || !data->hex_expected_iv || 
        !data->hex_plaintext || !data->hex_expected_ct) {
        printf("Nekompletne testovacie data\n");
        return false;
    }

    size_t lens[] = {
        strlen(data->hex_key) / 2,          // Kluc (pozor: v SIV musi byt dvojnasobnej dlzky oproti AES)
    ...
    if (lens[0] != 2 * (AES_KEY_SIZE)) {
        printf("Chyba: Nespravna dlzka kluca pre SIV rezim. Ocakavana dlzka: %d bajtov (dvojnasobok AES kluca)\n", 
               2 * AES_KEY_SIZE);
        return false;
    }
    ...
    if (!actual_iv || !actual_ct || !decrypted_pt) goto cleanup;
    ...
    const char *hexs[] = {
        data->hex_key, data->hex_expected_iv, data->hex_ad, 
        data->hex_plaintext, data->hex_expected_ct, NULL
    };

    for (int i = 0; i < 5; i++) {
        if (hexs[i] && hex_to_bin(hexs[i], bufs[i], lens[i]) != 0) goto cleanup;
    }

    printf("=== Test #%d ===\n", data->count);
    printf("Vstupne data:\n");
    printf("  Kluc (%zu bajtov - dvojnasobna dlzka kluca pre SIV): ", lens[0]);
    ...
    printf("  Plaintext: ");
    print_limited(data->hex_plaintext ? data->hex_plaintext : "(prazdny)", 75);
    ...
    AES_SIV_encrypt(bufs[0], bufs[3], lens[3], bufs[2], lens[2], iv_buffer, actual_ct);
    ...
    if (encrypt_ok) (*passed_encrypt)++;
    printf("  Vysledok sifrovania: %s\n", encrypt_ok ? "USPESNY" : "NEUSPESNY");

    // Test desifrovania (decrypt)
    printf("\nTest desifrovania:\n");
    
    uint8_t decrypt_status = AES_SIV_decrypt(bufs[0], bufs[1], bufs[4], lens[4], bufs[2], lens[2], decrypted_pt);

    bool auth_ok = (decrypt_status == 0);
    bool decrypt_match = auth_ok && (memcmp(decrypted_pt, bufs[3], lens[3]) == 0);
    ...
    if (decrypt_match) (*passed_decrypt)++;
...
int main() {
    ...
    int total_tests = processed_tests * 2;  // 1 sifrovanie + 1 desifrovanie pre kazdy test
