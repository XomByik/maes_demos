...
bool parse_header(FILE *fp, size_t *Alen, size_t *Plen) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strstr(trimmed, CCM_PREFIX_COUNT) || strstr(trimmed, CCM_PREFIX_KEY)) {
        ...
        if ((value_ptr = get_value_after_prefix(trimmed, CCM_PREFIX_ALEN))) {
            *Alen = strtoul(value_ptr, NULL, 10);
            alen_found = true;
        } else if ((value_ptr = get_value_after_prefix(trimmed, CCM_PREFIX_PLEN))) {
            *Plen = strtoul(value_ptr, NULL, 10);
            plen_found = true;
        } else if ((value_ptr = get_value_after_prefix(trimmed, CCM_PREFIX_NLEN))) {
            Nlen_file = strtoul(value_ptr, NULL, 10);
            nlen_found = true;
        } else if ((value_ptr = get_value_after_prefix(trimmed, CCM_PREFIX_TLEN))) {
        ...
        if (alen_found && plen_found && nlen_found && tlen_found) {
            printf("Hlavicka: Alen=%zu, Plen=%zu, Nlen=%zu, Tlen=%zu\n", 
                   *Alen, *Plen, Nlen_file, Tlen_file);
            
            if (Nlen_file != CCM_DEMO_NONCE_LEN || Tlen_file != CCM_DEMO_TAG_LEN) {
                printf("Chyba: Nlen/Tlen v subore (%zu/%zu) != Kompilovane (%zu/%zu)\n",
...
bool parse_next_test_case(FILE *fp, TestCaseData *data, uint8_t *key, int key_size_bytes) {
    char line[CCM_LINE_BUFFER_SIZE];
    char* value_ptr;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (in_test_case) {
            if ((value_ptr = get_value_after_prefix(trimmed, CCM_PREFIX_NONCE))) {
                data->hex_nonce = my_strdup(value_ptr);
            } else if ((value_ptr = get_value_after_prefix(trimmed, CCM_PREFIX_ADATA))) {
                data->hex_adata = my_strdup(value_ptr);
            } else if ((value_ptr = get_value_after_prefix(trimmed, CCM_PREFIX_PAYLOAD))) {
                data->hex_payload = my_strdup(value_ptr);
            } else if ((value_ptr = get_value_after_prefix(trimmed, CCM_PREFIX_CT))) {
                data->hex_ct_tag = my_strdup(value_ptr);
                return true;
            }
        }
    }
...
bool process_test_case(int test_num, const uint8_t* key, const TestCaseData* data,
                      size_t Alen, size_t Plen, 
    ...
    bool conversion_ok = (hex_to_bin(data->hex_nonce, current_nonce, CCM_DEMO_NONCE_LEN) == 0);
    if (Alen > 0) {
        conversion_ok &= (hex_to_bin(data->hex_adata, current_adata, Alen) == 0);
    }
    if (Plen > 0) {
        conversion_ok &= (hex_to_bin(data->hex_payload, current_payload, Plen) == 0);
    }
    if (ct_tag_len > 0) {
        conversion_ok &= (hex_to_bin(data->hex_ct_tag, current_ct_tag, ct_tag_len) == 0);
    }
    ...
    printf("\nTest sifrovania:\n");
    AES_CCM_encrypt(key, current_nonce,
                    current_payload, Plen,
                    current_adata, Alen,
    ...
    bool encrypt_ok = (Plen == 0 || memcmp(result_ciphertext, expected_ct, Plen) == 0) &&
    ...
    printf("\nTest desifrovania:\n");
    uint8_t decrypt_status = AES_CCM_decrypt(key, current_nonce,
                                           current_ct_tag, Plen,
                                           current_adata, Alen,
                                           CCM_DEMO_TAG_LEN,
                                           result_plaintext);
...
int main() {
    ...
    if (!parse_header(fp, &Alen, &Plen) || !parse_initial_key(fp, key, key_size_bytes)) {
    ...
    while (parse_next_test_case(fp, &current_test, key, key_size_bytes)) {
        tests_total++;
        process_test_case(current_test.count, key, &current_test,
                         Alen, Plen,
    ...
    success = (tests_total > 0 && 
              tests_passed_encrypt == tests_total && 
