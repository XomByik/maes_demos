...
#include "common.h"
#include "cfb_config.h"

static int get_segment_size(const char* filename) {
    if (strstr(filename, "cfb1_") != NULL) return 1;
    else if (strstr(filename, "cfb8_") != NULL) return 8;
    else return 128;
}

static LineType get_line_type(const char* line) {
    if (strncmp(line, "Key", 3) == 0) return KEY;
    if (strncmp(line, "IV", 2) == 0) return IV;
    if (strncmp(line, "Segment #", 9) == 0) return SEGMENT;
    ...
    if (strstr(line, "CFB") != NULL) return MODE_CHANGE;
    return UNKNOWN;
}
...
bool process_test_case(const TestCaseData *data, uint8_t* key, uint8_t* iv, int* passed_count) {
    ...
    if (data->segment_size == 1) {
        ...
        if (data->is_encrypt) {
            ...
            process_cfb(key, iv, &plaintext_bit, &result_bit, data->segment_size, true);
            ...
            process_cfb(key, iv, &ciphertext_bit, &result_bit, data->segment_size, false);
        ...
        if (data->is_encrypt) {
            ...
            process_cfb(key, iv, &plaintext_byte, &result_byte, data->segment_size, true);
            ...
            process_cfb(key, iv, &expected_ciphertext_byte, &result_byte, data->segment_size, false);
        ...
        if (data->is_encrypt) {
            ...
            process_cfb(key, iv, plaintext_bytes, result_bytes, data->segment_size, true);
            ...
            process_cfb(key, iv, expected_ciphertext_bytes, result_bytes, data->segment_size, false);
...
bool parse_test_data(FILE *fp, TestCaseData *data, uint8_t *key, uint8_t *iv, uint8_t *original_iv, 
                   int *test_count, int *passed_count, int segment_size, bool *first_segment_in_file) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        switch(type) {
            ...
            case IV:
                free(data->hex_iv);
                data->hex_iv = my_strdup(trim(line + 3));
            ...
            case INPUT_BLOCK:
                free(data->hex_input_block);
                data->hex_input_block = my_strdup(trim(line + 12));
            ...
            case OUTPUT_BLOCK:
                free(data->hex_output_block);
                data->hex_output_block = my_strdup(trim(line + 12));
                break;
                
            case PLAINTEXT:
                free(data->plaintext_str);
                data->plaintext_str = my_strdup(trim(line + 9));
                break;
                
            case CIPHERTEXT:
                free(data->ciphertext_str);
                data->ciphertext_str = my_strdup(trim(line + 10));
                
                if (data->hex_key && data->hex_iv && data->hex_input_block && data->hex_output_block && 
                    data->plaintext_str && data->ciphertext_str) {
...
int main() {
    ...
    for (int file_idx = 0; file_idx < 3; file_idx++) {
        ...
        parse_test_data(fp, &test_data, key, iv, original_iv, &test_count, &passed_count,
                      segment_size, &first_segment_in_file);
