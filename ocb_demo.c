#include "ocb_config.h"

typedef enum {
    KEY, NONCE, AAD, PT, CT, COUNT, FAIL
} LineType;

static LineType get_line_type(const char* line) {
    if (strstr(line, "K : ")) return KEY;
    if (strncmp(line, "N:", 2) == 0 || strncmp(line, "N : ", 4) == 0) return NONCE;
    if (strncmp(line, "A:", 2) == 0 || strncmp(line, "A : ", 4) == 0) return AAD;
    if (strncmp(line, "P:", 2) == 0 || strncmp(line, "P : ", 4) == 0) return PT;
    if (strncmp(line, "C:", 2) == 0 || strncmp(line, "C : ", 4) == 0) return CT;
...
static char* get_ocb_value(const char* line, const char* short_prefix, const char* long_prefix) {
    char* value = NULL;
    char* trimmed = NULL;
    
    if (strncmp(line, long_prefix, strlen(long_prefix)) == 0) {
        value = my_strdup(line + strlen(long_prefix));
        if (value) {
            trimmed = trim(value);
            if (trimmed != value) {
        ...
        value = my_strdup(line + strlen(short_prefix));
        if (value) {
            trimmed = trim(value);
            if (trimmed != value) {
                memmove(value, trimmed, strlen(trimmed) + 1);
        ...
        switch(type) {
            ...
            case NONCE:
                if (!in_test_case && global_key) {
                    ...
                    in_test_case = true;
                    data->hex_key = my_strdup(global_key);
...
bool process_test_case(const TestCaseData *data, int *passed_encrypt, int *passed_decrypt) {
    ...
    // Test sifrovania (encrypt)
    {
        ...
        bool ct_match = (lens[4] == 0 || memcmp(res_ct, bufs[4], lens[4]) == 0);
    ...
    // Test desifrovania (decrypt)
    {
        ...
        bool decrypt_ok = auth_success && (lens[3] == 0 || memcmp(decrypted, bufs[3], lens[3]) == 0);
...
int main() {
    ...
    int total_tests = processed_tests * 2; // Každý test má encrypt aj decrypt časť
