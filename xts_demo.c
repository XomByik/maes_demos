#include "xts_config.h"

typedef enum {
    KEY1_T, KEY2_T, TWEAK_T, PTX_T, CTX_T, COUNT_T, FAIL_T
} LineType;

static LineType get_line_type(const char* line) {
    if (strstr(line, "Key1")) return KEY1_T;
    if (strstr(line, "Key2")) return KEY2_T;
    if (strstr(line, "DUCN") || strstr(line, "Tweak")) return TWEAK_T;  // Pridané rozpoznanie DUCN
    if (strstr(line, "PTX")) return PTX_T;
    if (strstr(line, "CTX")) return CTX_T;
...
static char* get_line_value(const char* line, const char* prefix) {
    ...
    start += strlen(start) - strlen(strchr(start, ' '));  // presunúť na medzeru
    while (isspace(*start)) start++;  // preskočiť medzery
...
bool parse_next_test_case(FILE *fp, TestCaseData *data) {
    ...
    static int current_count = 0; // Statické počítadlo pre automatické číslovanie
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (!trimmed || strlen(trimmed) == 0 || trimmed[0] == '#' || trimmed[0] == '/') {
            if (in_test_case && in_ctx_section && data->hex_key1 && data->hex_key2 && 
                ...
                if (data->count == 0) data->count = ++current_count; // Automatické číslovanie
                return true;
            }
            // Ak máme všetky dáta a prázdny riadok, pravdepodobne koniec testu
            if (in_test_case && data->hex_key1 && data->hex_key2 && data->hex_tweak && 
        ...
        switch(type) {
            ...
            case KEY2_T:
                ...
                if (!data->hex_key2) data->hex_key2 = value; else free(value);
                break;

            case TWEAK_T:
                value = get_line_value(trimmed, "DUCN");  // Skúsime najprv DUCN
                if (!value) value = get_line_value(trimmed, "Tweak");  // Ak neúspešne, skúsime Tweak
                if (!data->hex_tweak) data->hex_tweak = value; else free(value);
                break;
            ...
            case PTX_T:
                ...
                if (!data->hex_plaintext) {
                    ...
                    char* new_ptx = realloc(data->hex_plaintext, current_len + append_len + 1);
                    if (new_ptx) {
            ...
            case CTX_T:
                ...
                if (!data->hex_ciphertext) {
                    ...
                    size_t append_len = strlen(value);
                    char* new_ctx = realloc(data->hex_ciphertext, current_len + append_len + 1);
                    if (new_ctx) {
...
bool process_test_case(const TestCaseData *data, int *passed_count) {
    ...
    size_t lens[] = {
        strlen(data->hex_key1) / 2,
        strlen(data->hex_key2) / 2,
    ...
    const char *hexs[] = {
        data->hex_key1, data->hex_key2, data->hex_tweak, 
        data->hex_plaintext, data->hex_ciphertext, NULL
    };

    for (int i = 0; i < 5; i++) {
        if (hex_to_bin(hexs[i], bufs[i], lens[i]) != 0) goto cleanup;
    }
    ...
    printf("\nTest sifrovania:\n");
    char status = AES_XTS_encrypt(bufs[5], bufs[2], bufs[3], lens[3], result);
