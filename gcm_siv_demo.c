#include "gcm_siv_config.h"

typedef enum {
    KEY, NONCE, AAD, PT, CT, COUNT
} LineType;

static LineType get_line_type(const char* line) {
    if (strstr(line, "key = ")) return KEY;
    if (strstr(line, "iv = ")) return NONCE;
    if (strstr(line, "aad = ")) return AAD;
        ...
        switch(type) {
            ...
            case NONCE:
                if (in_test_case) {
                    ...
                    if (!data->hex_nonce) data->hex_nonce = value; else free(value);
                }
                break;
            ...
            case AAD:
                if (in_test_case) {
                    ...
                    if (!data->hex_aad) data->hex_aad = value; else free(value);
                }
                break;
                ...
                if (in_test_case) {
                    ...
                    if (value) {
                        ...
                        if (combined_len >= tag_hex_len) {
                            ...
                            data->hex_ciphertext = strndup(value, ct_hex_len);
...
finish_test_case:
    if (in_test_case && data->hex_key && data->hex_nonce && data->hex_tag) {
        ...
        if (!data->hex_aad) data->hex_aad = strdup("");
        if (!data->hex_plaintext) data->hex_plaintext = strdup("");
        if (!data->hex_ciphertext) data->hex_ciphertext = strdup("");
...
bool process_test_case(const TestCaseData *data, int *passed_encrypt, int *passed_decrypt) {
    ...
    // Validate lengths
    if (lens[0] != 16 && lens[0] != 24 && lens[0] != 32) return false;
    if (lens[1] != GCM_SIV_NONCE_LEN) return false;
    if (lens[5] != GCM_SIV_TAG_LEN) return false;
    if (lens[4] != lens[3]) return false;
    ...
    if (!result_ct || !result_tag || !combined || !decrypted) goto cleanup;

    const char *hexs[] = {
        data->hex_key, data->hex_nonce, data->hex_aad,
        data->hex_plaintext, data->hex_ciphertext, data->hex_tag
    };
    ...
    printf("=== Test #%d ===\n", data->count);
    printf("Vstupne data:\n");
    printf("  Nonce: "); print_limited(data->hex_nonce, 75);
    if (data->hex_aad && strlen(data->hex_aad) > 0) {
        printf("  AAD: "); print_limited(data->hex_aad, 75);
    } else {
        printf("  AAD: (prazdne)\n");
    }
    printf("  Plaintext: "); print_limited(data->hex_plaintext ? data->hex_plaintext : "(prazdny)", 75);

    printf("\nTest sifrovania:\n");
    GCM_SIV_encrypt(bufs[0], bufs[1], bufs[3], lens[3],
                    bufs[2], lens[2], result_ct, result_tag);

    printf("  Vypocitany ciphertext: "); print_hex(result_ct, lens[3]);
    printf("  Ocakavany ciphertext: "); print_hex(bufs[4], lens[4]);
    printf("  Vypocitany tag: "); print_hex(result_tag, GCM_SIV_TAG_LEN);
    printf("  Ocakavany tag: "); print_hex(bufs[5], lens[5]);
    ...
    printf("\nTest desifrovania:\n");
    if (lens[4] > 0) memcpy(combined, bufs[4], lens[4]);
    memcpy(combined + lens[4], bufs[5], lens[5]);

    uint8_t decrypt_status = GCM_SIV_decrypt(bufs[0], bufs[1], combined, lens[4],
    ...
    printf("  Ocakavany plaintext: "); print_hex(bufs[3], lens[3]);
...
int main() {
    ...
    bool success = (processed_tests > 0 &&
