...
static LineType get_line_type(const char *line) {
  ...
  if (strstr(line, "FAIL"))
    return FAIL;
  if (strstr(line, "[PLAINTEXT LENGTH = "))
...
bool process_test_case(const TestCaseData *data, int *passed_count) {
  ...
  if (data->is_unwrap) {
    ...
    if (success)
      (*passed_count)++; // len inkrementujeme passed_count
  } else {
    // Wrap operation
    printf("  Plaintext: %s\n", data->hex_plaintext);
    if (data->hex_ciphertext) {
...
int main() {
  ...
  bool all_passed =
      (wrap_passed == wrap_total) && (unwrap_passed == unwrap_total);
