#include "ecb_config.h"

typedef enum {
    KEY, PLAINTEXT, CIPHERTEXT, BLOCK, MODE_CHANGE, UNKNOWN
} LineType;
...
static LineType get_line_type(const char* line) {
    ...
    if (strstr(line, "ECB-AES") != NULL) return MODE_CHANGE;
    return UNKNOWN;
}
...
bool parse_next_test_case(FILE *fp, TestCaseData *data) {
    ...
    char *prev_key = data->hex_key ? my_strdup(data->hex_key) : NULL;
    free_test_case_data(data);
    
    // Nastav kľúč z predchádzajúceho behu alebo globálneho kľúča
    data->hex_key = prev_key ? prev_key : (current_key ? my_strdup(current_key) : NULL);
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        switch(type) {
            ...
            case BLOCK:
                ...
                if (in_test_case && data->hex_key && data->hex_plaintext && data->hex_ciphertext) {
                    fseek(fp, start_pos, SEEK_SET);
                    if (!data->count) data->count = ++current_count;
                    return true;
                }
        ...
        if (data->hex_key && data->block_number > 0 && 
...
bool process_test_case(const TestCaseData *data, int *passed_encrypt, int *passed_decrypt) {
    ...
    size_t pt_len = data->hex_plaintext ? strlen(data->hex_plaintext) / 2 : 0;
    size_t ct_len = data->hex_ciphertext ? strlen(data->hex_ciphertext) / 2 : 0;
