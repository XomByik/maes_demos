...
static LineType get_line_type(const char* line) {
    ...
    if (strstr(line, "CBC-AES") != NULL) return MODE_CHANGE;
    return UNKNOWN;
}
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        switch(type) {
            ...
            case PLAINTEXT:
                if (encrypt_mode) {
                    free(data->hex_plaintext);
                    data->hex_plaintext = my_strdup(trim(line + 10));
                } else {
                    free(data->hex_plaintext);
                    data->hex_plaintext = my_strdup(trim(line + 10));
                    
                    if (data->hex_key && data->hex_iv && data->hex_ciphertext && data->hex_plaintext) {
                        (*test_count)++;
                        data->count = *test_count;
                        data->is_encrypt = encrypt_mode;
                        
                        process_test_case(data, key, iv, prev_ciphertext, passed_count, is_first_block);
                    }
                }
                break;
            ...
            case CIPHERTEXT:
                if (!encrypt_mode) {
                    free(data->hex_ciphertext);
                    data->hex_ciphertext = my_strdup(trim(line + 11));
                } else {
                    free(data->hex_ciphertext);
                    data->hex_ciphertext = my_strdup(trim(line + 11));
                    
                    if (data->hex_key && data->hex_iv && data->hex_plaintext && data->hex_ciphertext) {
                        (*test_count)++;
                        data->count = *test_count;
                        data->is_encrypt = encrypt_mode;
...
int main() {
    ...
    parse_test_data(fp, &test_data, key, iv, prev_ciphertext, &test_count, &passed_count, &is_first_block);
