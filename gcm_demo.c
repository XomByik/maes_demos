    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        switch(type) {
            ...
            case IV:
                ...
                if (!data->hex_iv) data->hex_iv = value; else free(value);
                break;
            ...
            case TAG:
                ...
                if (!data->hex_tag) data->hex_tag = value; else free(value);
                break;
...
bool process_test_case(const TestCaseData *data, int *passed_encrypt, int *passed_decrypt) {
    ...
    const char *hexs[] = {
        data->hex_key, data->hex_iv, data->hex_aad, 
    ...
    if (data->is_decrypt) {
        ...
        bool ok = data->should_fail ? 
                 ...
                 (status == NO_ERROR_RETURNED && 
                  (!data->hex_plaintext || memcmp(decrypted, bufs[3], lens[3]) == 0));
        ...
        print_limited(data->hex_ciphertext ? data->hex_ciphertext : "(ziadny)", 75);
        ...
        AES_GCM_encrypt(bufs[0], bufs[1], bufs[3], lens[3],
                       bufs[2], lens[2], res_ct, res_tag);
        ...
        bool ct_match = (!data->hex_ciphertext || memcmp(res_ct, bufs[4], lens[4]) == 0);
